    ...
    if message.has(At) and message_text.startswith("摸") or message_text.startswith("摸 "):
        ...
        try:
            await app.sendGroupMessage(
                group,
                MessageChain.create([
                    Image.fromLocalFile(path.join(path.plugin,
                                                  f"PetPet/temp",
...
frames = tuple([path.join(path.plugin, f'PetPet/PetPetFrames/frame{i}.png') for i in range(5)])
...
async def make_frame(avatar, i, squish=0, flip=False):
    ...
    # 将头像放缩成所需大小
    avatar = avatar.resize((int((spec[2] - spec[0]) * 1.2), int((spec[3] - spec[1]) * 1.2)),
                           Image.ANTIALIAS)
    # 并贴到空图像上
...
async def petpet(member_id, flip=False, squish=0, fps=20) -> None:
    ...
    # 输出
    await save_gif(gif_frames, path.join(path.plugin,
                                         f'PetPet/temp/tempPetPet-{member_id}.gif'), fps=fps)
