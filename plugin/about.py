...
async def about(event: CommandEvent):
    if event.perm_lv >= permission.USER and event.command.startswith(".about"):
...
async def xenon_help(event: CommandEvent):
    if event.perm_lv >= permission.USER and event.command.startswith(".help"):
        if (
            len(event.command.split()) == 2
            and (name := event.command.split()[1]) in plugins.loaded.keys()
        ):
