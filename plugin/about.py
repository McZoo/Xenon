...
plugin_spec = lib.plugin.XenonPluginSpec(lib.Version(0, 1, 0), "about", "BlueGlassBlock", ".help： Xenon 的帮助菜单\n"
                                                                                          ".about： 关于Xenon的信息")
...
def main(ctx: lib.XenonContext):
    ...
    async def xenon_help(event: CommandEvent):
        if event.user and event.perm_lv >= permission.USER and event.command.startswith(".help"):
            if len(event.command.split()) == 2 and (name := event.command.split()[1]) in ctx.plugins.loaded.keys():
