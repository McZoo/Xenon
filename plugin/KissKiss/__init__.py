...
async def petpet_generator(app: GraiaMiraiApplication, message: MessageChain, group: Group,
    ...
    if (message.has(At)
        ...
        if member.id == target:
            await app.sendGroupMessage(group, MessageChain.create([Plain("请不要自交~😋")]),
                                       quote=message[Source][0])
        else:
            pic = path.join(path.plugin, f"KissKiss/temp/tempKiss-{member.id}-{target}.gif")
            await kiss(member.id, target)
            await app.sendGroupMessage(group, MessageChain.create([Image.fromLocalFile(pic)]))
