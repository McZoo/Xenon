...
async def pornhub_style_logo_generator(event: CommandEvent):
    try:
        ...
        try:
            await event.send_result(await make_ph_style_logo(left_text, right_text))
        except AccountMuted:
            pass
    except ValueError:
        try:
            await event.send_result(MessageChain.create(
                [Plain(text="参数非法！使用格式：.ph text1 text2")]))
        except AccountMuted:
            pass
...
async def create_left_part_img(text: str, font_size: int):
    ...
    font = ImageFont.truetype(path.join(path.plugin, 'PHLogoGen/ttf/ArialEnUnicodeBold.ttf'),
                              font_size)
...
async def create_right_part_img(text: str, font_size: int):
    ...
    alpha.paste(
        circle.crop((magnified_radii, magnified_radii, magnified_radii * 2, magnified_radii * 2)),
...
async def combine_img(left_text: str, right_text, font_size: int, out_put_path: str):
    ...
    bg_img.paste(right_img, (blank + left_img.width, int((bg_img_height - right_img.height) / 2)),
                 mask=right_img)
